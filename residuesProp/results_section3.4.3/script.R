...
#			the high and low standard deviation sets for division set 1.
#
###################### 
...
highSD<-read.table("freediv1propH.txt", header=TRUE, sep="\t")
head(highSD)

lowSD<-read.table("freediv1propL.txt", header=TRUE, sep="\t")
head(lowSD)
...
boxplot(highHydro,lowHydro,names=c("SD>3","SD<1"),main="Hydrophobicity")
...
boxplot(highMW,lowMW,names=c("SD>3","SD<1"),main="Molecular weight")

t.test(highPI,lowPI,var=T)
boxplot(highPI,lowPI,names=c("SD>3","SD<1"),main="Isoelectric point")
...
#Types amino acids
typeH<-read.table("freeRdiv1typeH.txt", header=TRUE, sep="\t")
head(typeH)

typeL<-read.table("freeRdiv1typeL.txt", header=TRUE, sep="\t")
head(typeL)
...
boxplot(phobicH,phobicL,names=c("SD>3","SD<1"),main="Hydrophobic residues")
boxplot(philicH,philicL,names=c("SD>3","SD<1"),main="Hydrophilic residues")
boxplot(amphiH,amphiL,names=c("SD>3","SD<1"),main="Amphipathic residues")
boxplot(glyH,glyL,names=c("SD>3","SD<1"),main="Glycines residues")
...
# Glycines aa
plot(sdTypes/10,glyTypes,main="Glycines abundance vs SD",
