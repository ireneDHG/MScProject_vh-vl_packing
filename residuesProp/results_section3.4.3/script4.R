...
#			the high and low standard deviation sets for division set 4.
#
###################### 
...
highSD<-read.table("freediv4propH.txt", header=TRUE, sep="\t")
head(highSD)

lowSD<-read.table("freediv4propL.txt", header=TRUE, sep="\t")
head(lowSD)
...
boxplot(highHydro,lowHydro,names=c("SD>5","SD<2"),main="Hydrophobicity")
...
boxplot(highMW,lowMW,names=c("SD>5","SD<2"),main="Molecular weight")

t.test(highPI,lowPI,var=T)
boxplot(highPI,lowPI,names=c("SD>5","SD<2"),main="Isoelectric point")
...
#Types amino acids
typeH<-read.table("freeRdiv4typeH.txt", header=TRUE, sep="\t")
head(typeH)

typeL<-read.table("freeRdiv4typeL.txt", header=TRUE, sep="\t")
head(typeL)
...
boxplot(phobicH,phobicL,names=c("SD>5","SD<2"),main="Hydrophobic residues")
boxplot(philicH,philicL,names=c("SD>5","SD<2"),main="Hydrophilic residues")
boxplot(amphiH,amphiL,names=c("SD>5","SD<2"),main="Amphipathic residues")
boxplot(glyH,glyL,names=c("SD>5","SD<2"),main="Glycines residues")
