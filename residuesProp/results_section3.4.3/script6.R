...
#			the high and low standard deviation sets for division set 6.
#
###################### 
...
highSD<-read.table("freediv6propH.txt", header=TRUE, sep="\t")
head(highSD)

lowSD<-read.table("freediv6propL.txt", header=TRUE, sep="\t")
head(lowSD)
...
boxplot(highHydro,lowHydro,names=c("SD>4","SD<1"),main="Hydrophobicity")
...
boxplot(highMW,lowMW,names=c("SD>4","SD<1"),main="Molecular weight")

t.test(highPI,lowPI,var=T)
boxplot(highPI,lowPI,names=c("SD>4","SD<1"),main="Isoelectric point")
...
#Types amino acids
typeH<-read.table("freeRdiv6typeH.txt", header=TRUE, sep="\t")
head(typeH)

typeL<-read.table("freeRdiv6typeL.txt", header=TRUE, sep="\t")
head(typeL)
...
boxplot(phobicH,phobicL,names=c("SD>4","SD<1"),main="Hydrophobic residues")
boxplot(philicH,philicL,names=c("SD>4","SD<1"),main="Hydrophilic residues")
boxplot(amphiH,amphiL,names=c("SD>4","SD<1"),main="Amphipathic residues")
boxplot(glyH,glyL,names=c("SD>4","SD<1"),main="Glycines residues")
