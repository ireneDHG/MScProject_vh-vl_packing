...
highSD<-read.table("div3propH.txt", header=TRUE, sep="\t")
head(highSD)

lowSD<-read.table("div3propL.txt", header=TRUE, sep="\t")
head(lowSD)
...
#Types amino acids
typeH<-read.table("Rdiv3typeH.txt", header=TRUE, sep="\t")
head(typeH)

typeL<-read.table("Rdiv3typeL.txt", header=TRUE, sep="\t")
head(typeL)
